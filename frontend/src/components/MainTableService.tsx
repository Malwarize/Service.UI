...
import {FetchServices, RestartService, StartService, StopService} from '../../wailsjs/go/main/App';
...
function MainTableService({ searchQuery }: { searchQuery: string }) {
    ...
    return (
        <div className='h-[calc(100vh-10rem)] overflow-y-scroll overflow-visible'>
            <SortableTable rows={filteredServices} columns={
                [
                    {"field": "Name", "headerName": "Service","width": 300,"renderCell" : (params:any)=> (<p className="font-bold">{params.value}</p>)},
